...
public abstract class ResourceSubtypeControllerBase<TGetQuery, TSearchQuery, TCreateCommand, TUpdateCommand, TRemoveCommand> : BaseController
    where TGetQuery : class, IQuery<ResourceSubtypeDto>
    where TSearchQuery : class, IQuery<IEnumerable<ResourceSubtypeDto>>
    where TCreateCommand : class, ICreateResourceSubtypeCommand
    where TUpdateCommand : class, IUpdateResourceSubtypeCommand
    where TRemoveCommand : class, IRemoveResourceSubtypeCommand
{
    ...
    protected ResourceSubtypeControllerBase(
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<ResourceSubtypeDto>> Get([FromRoute] TGetQuery query)
    {
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<ResourceSubtypeDto>>> Get([FromQuery] TSearchQuery query)
    {
        var result = await _queryDispatcher.QueryAsync(query);
        return Ok(result);
    }
